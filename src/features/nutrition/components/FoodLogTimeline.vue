...
import type { FoodLogHourGroup } from '../lib/foodLogTimeline'
...
const hours = computed(() => timelineHours(groups.value))

function hourLabel(hour: number): string {
  return format(new Date(2000, 0, 1, hour), 'p', { locale: dateLocale })
}

function entryTimeLabel(entry: DbNutritionDiaryEntry): string {
  return isLoggedOnDiaryDay(entry)
    ? format(new Date(entry.loggedAt), 'p', { locale: dateLocale })
...
function groupFor(hour: number): FoodLogHourGroup | undefined {
  return groupsByHour.value.get(hour)
}
</script>
...
<template>
  <ol class="relative" data-testid="food-log-timeline">
    ...
    <li v-for="hour in hours" :key="hour" class="relative flex gap-3 py-1.5">
      ...
      <div class="min-w-0 flex-1 pr-1">
        <div class="flex min-h-8 items-center justify-between gap-2">
          <button
            type="button"
            class="flex size-7 items-center justify-center rounded-full bg-muted text-muted-foreground transition-colors hover:bg-accent hover:text-foreground"
            :aria-label="t('nutrition.foodLog.addAtHour', { hour: hourLabel(hour) })"
            @click="emit('add', mealForHour(hour))"
          >
          ...
          >
            {{ macroSummary(groupFor(hour)!.totals) }}
          </p>
        </div>

        <ul v-if="groupFor(hour)" class="mt-1 space-y-2 pb-1" role="list">
          <li
            v-for="entry in groupFor(hour)!.entries"
            :key="entry.id"
          ...
          >
            ...
            <button
              type="button"
              class="shrink-0 rounded-lg p-2 text-muted-foreground transition-colors hover:bg-muted hover:text-destructive"
