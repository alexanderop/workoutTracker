    ...
    >
      ...
      <DialogHeader>
        <div class="flex items-center gap-2">
          ...
          <DialogTitle>{{ t('dialogs.fortimeConfig.title') }}</DialogTitle>
        </div>
        <DialogDescription>{{ t('dialogs.fortimeConfig.description') }}</DialogDescription>
      </DialogHeader>
      ...
      <div class="flex-1 overflow-y-auto space-y-6 py-4">
        ...
        <div class="space-y-3">
          ...
          <p v-if="exercises.length === 0" class="text-center py-6 text-muted-foreground">
            {{ t('dialogs.fortimeConfig.noExercises') }}
          </p>
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="flex gap-2 mt-1">
                <Input
                  ...
                  min="1"
                  class="h-8 w-20"
                  :placeholder="t('dialogs.fortimeConfig.repPlaceholder')"
                  @update:model-value="updateExerciseReps(index, Number($event))"
                />
                <Input
                  :model-value="exercise.load ?? ''"
                  class="h-8 flex-1"
                  :placeholder="t('dialogs.fortimeConfig.loadPlaceholder')"
          ...
          <Button variant="outline" class="w-full" @click="showExercisePicker = true">
            ...
            {{ t('dialogs.fortimeConfig.addExercise') }}
          </Button>
        </div>
      </div>
      ...
      <div class="pt-4 border-t flex gap-3">
        ...
        <Button class="flex-1" :disabled="!canConfirm" @click="handleConfirm">{{
          t('dialogs.fortimeConfig.addBlock')
        }}</Button>
      </div>
    </MobileDialogContent>
  </Dialog>
