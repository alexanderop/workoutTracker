...
import { Trophy, Check } from '@lucide/vue'
...
type Properties = {
  ...
  /** Duration in seconds */
  duration: number
  /** Optional override for duration label (defaults to "Total Duration") */
...
const displayLabel = computed(() => durationLabel ?? t('workouts.summary.totalDuration'))
</script>
...
<template>
  <div class="flex-1 flex flex-col items-center justify-center p-6 gap-8">
    ...
    >
      ...
      <p class="text-muted-foreground mt-2">{{ displayLabel }}</p>
    </div>
