...
describe('workoutBlockStatus', () => {
  it('treats strength blocks as complete only when every set is completed', () => {
    const complete = createStrengthBlockWithSets([
      { status: 'completed' },
      { status: 'completed' },
    ])
    const incomplete = createStrengthBlockWithSets([
      { status: 'completed' },
      { status: 'planned' },
    ])
  ...
  it('treats non-strength blocks as complete when they have a result', () => {
    ...
    expect(isWorkoutBlockComplete({ ...amrap, result: { rounds: 4, partialReps: 12, actualDuration: 600 } })).toBe(true)
    expect(isWorkoutBlockComplete(cardio)).toBe(false)
    expect(isWorkoutBlockComplete({
      ...cardio,
      result: {
  ...
  it('finds the first incomplete block', () => {
    ...
    expect(findFirstIncompleteWorkoutBlockIndex([
      completeStrength,
      {
        ...completeCardio,
        result: {
