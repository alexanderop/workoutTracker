...
describe('Strength Workflows', () => {
  describe('Set Completion', () => {
    it('advances to the next set after completing a set', async ({ createTestApp }) => {
      const { builder, workout, common, getByRole } = await createTestApp()
    ...
    it('displays strength block UI and allows completing all sets', async ({ createTestApp }) => {
  ...
  describe('Value Prefilling', () => {
    it('prefills values from previous set when advancing', async ({ createTestApp }) => {
      const { builder, workout, getByRole } = await createTestApp()
      ...
      // Wait for table to render
      await expect.element(page.getByRole('table')).toBeVisible()
      await expect.element(getByRole('heading', { name: /squat/i })).toBeVisible()
