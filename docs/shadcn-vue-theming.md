...
Use it whenever a component accepts a `class` prop or you conditionally toggle utilities. `tailwind-merge` resolves conflicts so the *last* class wins (`cn('p-2', 'p-4')` → `'p-4'`).
